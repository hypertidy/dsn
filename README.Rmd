...
- prefixing `/vsicurl/` to an online data source url for [GDAL's Virtual File System](https://gdal.org/en/stable/user/virtual_file_systems.html)
- prefixing a driver declaration to a data source, e.g. `HDF5:/my/files/data.h5` or `NETCDF:C:/temp/nc/afile.nc`
- wrapping a [subdataset](https://gdal.org/en/stable/user/raster_data_model.html)  declaration, e.g. `NETCDF:/myfiles/data.nc:variable` that can occur in [NETCDF](https://gdal.org/en/stable/drivers/raster/netcdf.html) and similar formats
- wrapping a [VRT connection](https://gdal.org/en/stable/drivers/raster/vrt.html), e.g. `vrt://myfile.tif?bands=3,2,1`
...
`vrtcon()` is for a [VRT connection](https://gdal.org/en/stable/drivers/raster/vrt.html) string, a compact and dependency free syntax for augmenting GDAL data source names, there are new features coming for that in GDAL 3.7. 
